...
from data_processor import load_and_preprocess, get_global_stats, enrich_data
...
# app is constructed after lifespan is defined — see below

# TripPlanRequest (full version) defined later in this file
...
def geocode_nominatim(address: str):
    ...
    # Second try with ", India" suffix if not already present
    if "india" not in address.lower():
        try:
            url2 = f"https://nominatim.openstreetmap.org/search?q={address}, India&format=json&limit=1"
    ...
    if "thrissur" in lower_addr: return 10.5276, 76.2144
    if "kochi" in lower_addr or "cochin" in lower_addr: return 9.9312, 76.2673
    if "trivandrum" in lower_addr or "thiruvananthapuram" in lower_addr: return 8.5241, 76.9366
    
    return None, None

# CORS middleware registered below after app is created
...
congestion_model.train(df=df)

# Create simulator (started in lifespan below)
session_sim = SessionSimulator(congestion_model)

# ── FIX: Register lifespan properly so the simulator background loop starts ──
@asynccontextmanager
async def lifespan(app_):
    """FastAPI lifespan: starts the real-time session simulator on startup."""
    session_sim.start()
    print("[Lifespan] Session simulator started.")
    yield
    session_sim.stop()
    print("[Lifespan] Session simulator stopped.")

# ── FIX: Pass lifespan to FastAPI constructor (not patched after creation) ────
app = FastAPI(title="EV ChargeSync API", lifespan=lifespan)

# Enable CORS for frontend (must be after app is created)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def get_congestion_forecast():
    ...
    try:
        ...
        all_states = session_sim.get_all_states()
        # FIX: sparkline values are already 0-100 percentages.
        # Accumulate weighted sum of pct (weighted by charger count),
        # then divide by total charger count — no extra *100 needed.
        hourly_totals = [0.0] * 6
        hourly_capacity = [0] * 6

        for s in all_states:
            sid = s["station_id"]
            capacity = max(1, s["total_chargers"])
            pred = congestion_model.predict(sid, hours_ahead=0)
            # sparkline_6h values are already percentages (0-100)
            sparkline = pred.get("sparkline_6h", [s["congestion_pct"]] * 6)
            for i, pct in enumerate(sparkline[:6]):
                hourly_totals[i] += float(pct) * capacity
                hourly_capacity[i] += capacity

        # Weighted average percentage across all stations
        hourly_avg_pct = [
            int(hourly_totals[i] / max(1, hourly_capacity[i]))
            for i in range(6)
        ]
    ...
    try:
        ...
        for _, row in nearby.iterrows():
            ...
            try:
                ...
                # Mock a deterministic but realistic occupancy based on the station string so it doesn't jump around
                station_str = str(row.get("Station Name", "")) + str(hour)
                hash_val = int(hashlib.md5(station_str.encode()).hexdigest(), 16)
                
                # Peak hours 8am-11am and 4pm-8pm get a bump
                peak_bump = 30 if (8 <= hour <= 11) or (16 <= hour <= 20) else 0
                
                avg_occupancy_pct = (hash_val % 60) + 10 + peak_bump # 10% to ~100%
                avg_occupancy_pct = min(100, avg_occupancy_pct)
                
                # Assign 2 to 8 total chargers deterministically
                total_chargers = (hash_val % 7) + 2 
            except Exception:
                pass
            ...
            level = "Low" if avg_occupancy_pct < 40 else "Moderate" if avg_occupancy_pct < 70 else "High"
            
            # If high occupancy, generate a realistic 5-45m wait time
            wait_mins = 0 if level == "Low" else (hash_val % 40) + 5
...
def plan_trip(request: TripPlanRequest):
    ...
    # Load Indian EV stations from CSV instead of empty Database
    suggested = []
    ...
    try:
        ...
        min_lat = min(source_lat, dest_lat) - 2.0
        max_lat = max(source_lat, dest_lat) + 2.0
        min_lng = min(source_lng, dest_lng) - 2.0
        max_lng = max(source_lng, dest_lng) + 2.0
        ...
        for idx, row in df_map.iterrows():
            try:
                ...
                if min_lat <= lat <= max_lat and min_lng <= lng <= max_lng:
                    df_map_rows.append({
                        "Station Name": str(row['Station Name']) or f"Station-{idx}",
                ...
                pass
        with open("route_debug.txt", "w") as f:
            f.write(f"Loaded {len(df_map_rows)} stations in bounding box: min_lat={min_lat}, max_lat={max_lat}, min_lng={min_lng}, max_lng={max_lng}\n")
    except Exception as e:
    ...
    MAX_DIST_DEGREES = 0.1 # Roughly 10km depending on latitude
    ...
    with open("route_debug.txt", "a") as f:
        f.write(f"Matched {len(suggested)} stations near route\n")
    ...
    # Limit to 20 stations to avoid overcrowding
    ...
    for station in suggested:
        ...
        # Predict from model (often returns default or empty for Indian stations)
        try:
            cong = congestion_model.predict(sid, hours_ahead=1)
        except Exception:
            cong = {}

        # If the model didn't return valid availability, probabilistically mock it
        avail = cong.get("available_chargers")
        tot = cong.get("total_chargers")
        if avail is None or tot is None:
            hash_val = int(hashlib.md5(sid.encode()).hexdigest(), 16)
            tot = (hash_val % 7) + 2  # 2 to 8 chargers

            # Mock an occupancy percentage 10% to 90%
            occ_pct = (hash_val % 80) + 10
            active = round((occ_pct / 100) * tot)
            avail = max(0, tot - active)

            level = "Low" if occ_pct < 40 else "Moderate" if occ_pct < 70 else "High"
            wait_mins = 0 if level == "Low" else (hash_val % 40) + 5

            station["congestion_pct"] = occ_pct
            station["predicted_wait_minutes"] = wait_mins
            station["congestion_level"] = level
            station["available_chargers"] = avail
            station["total_chargers"] = tot
        else:
        ...
        wait_score = station.get("predicted_wait_minutes", 0)
...
def get_map_stations():
    """Returns all EV stations from the Indian EV CSV for map display."""
    try:
        import numpy as np
        df_map = pd.read_csv(r"d:\games\group11ziipp\group11\backend\Indian_EV_Stations_Simplified.csv")
        df_map = df_map.fillna("")
