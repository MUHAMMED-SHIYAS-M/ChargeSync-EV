...
def preprocess(path: str) -> pd.DataFrame:
    ...
    # ── Handle Pre-Aggregated Dataset (User provided) ─────────────────────────
    if "total_kWh" in df.columns and "session_count" in df.columns:
        print("[Preprocess] Detected pre-aggregated dataset. Adapting columns...")
        df = df.rename(columns={
            "hour": "hour_ts",
            "total_kWh": "total_kwh",
            "kWh_lag_1h": "kwh_lag1",
            "kWh_lag_2h": "kwh_lag2",
            "kWh_lag_24h": "kwh_lag24",
            "kWh_lag_168h": "kwh_lag168",
            "kWh_rolling_mean_3h": "kwh_roll3",
            "kWh_rolling_mean_6h": "kwh_roll6",
            "avg_session_hrs": "avg_duration"
        })
        df["hour_ts"] = pd.to_datetime(df["hour_ts"], utc=True, errors="coerce")
        if "avg_idle" not in df.columns:
            df["avg_idle"] = 0.0
        if "week_sin" not in df.columns and "week_of_year" in df.columns:
            df["week_sin"] = np.sin(2 * np.pi * df["week_of_year"] / 52)
            df["week_cos"] = np.cos(2 * np.pi * df["week_of_year"] / 52)
        
        # Drop rows with NaT hour_ts
        df = df.dropna(subset=["hour_ts"]).reset_index(drop=True)
        print(f"[Preprocess] {len(df):,} pre-aggregated hours ready.")
        return df

    # ── Original Raw Dataset Logic ───────────────────────────────────────────
...
def train_arima(df: pd.DataFrame):
    ...
    if "session_count" in df.columns:
        hourly = df.drop_duplicates(subset=["hour_ts"]).set_index("hour_ts")["session_count"].rename("sessions")
    else:
...
def train_lstm(df: pd.DataFrame, weather: dict | None = None):
    ...
    if "total_kwh" in df.columns and "kwh_lag168" in df.columns:
        # Pre-aggregated scenario
        hourly = df.drop_duplicates(subset=["hour_ts"]).set_index("hour_ts").asfreq("h").fillna(0).reset_index()
    else:
...
def build_hybrid_forecast(
    ...
    arima_24h = np.array(arima_24h)
    arima_kwh = np.array(arima_kwh)
    lstm_24h = np.array(lstm_24h)
...
def run_pipeline(data_path: str = DATA_PATH):
    ...
    col_name = "total_kwh" if "total_kwh" in df.columns else "kWhDelivered"
    avg_kwh = float(df[col_name].mean())
