...
class CongestionModel:
    ...
    def _preprocess_df(self, df: pd.DataFrame):
        """Applies feature engineering to the raw session dataframe."""
    ...
    def _train_xgboost(self):
        ...
        try:
            ...
            # Fix non-contiguous classes (e.g. if 1 is missing) using LabelEncoder
            le = LabelEncoder()
            ...
            # Inverse transform predictions for report match with labels
            y_test_labels = le.inverse_transform(y_test)
            y_pred_labels = le.inverse_transform(y_pred)
            
            from sklearn.metrics import confusion_matrix
            print("\n[CongestionModel] Confusion Matrix (XGBoost):")
            print(confusion_matrix(y_test_labels, y_pred_labels))
            
            print(classification_report(
                y_test_labels, y_pred_labels,
                target_names=[self.label_map.get(c, str(c)) for c in sorted(set(y_test_labels) | set(y_pred_labels))],
                zero_division=0,
            ))
    ...
    def train(self, df: pd.DataFrame = None):
        try:
            if df is not None:
                print("[CongestionModel] Using pre-loaded DataFrame …")
                self._preprocess_df(df)
            else:
